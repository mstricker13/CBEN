...
#python
#from transfer.datasets.BigEarthNet.bigearthnet_dataset_seco_lmdb_s2_uint8 import make_lmdb
#datapath = "/data/Phd_data/Projects/SSL/data/BigEarthNet/"
#datapath = "/data/Phd_data/Projects/SSL/data/BigEarthNet_CloudsR/"
#lmdbpathtrain = "/data/Phd_data/Projects/SSL/data/BigEarthNet_lmdb_trainNEW"
#lmdbpathtrain = "/data/Phd_data/Projects/SSL/data/BigEarthNet_lmdb_trainCloud"
#lmdbpathval = "/data/Phd_data/Projects/SSL/data/BigEarthNet_lmdb_valNEW"
#lmdbpathval = "/data/Phd_data/Projects/SSL/data/BigEarthNet_lmdb_valCloud"
#lmdbpathtest = "/data/Phd_data/Projects/SSL/data/BigEarthNet_lmdb_testNEW"
#lmdbpathtest = "/data/Phd_data/Projects/SSL/data/BigEarthNet_lmdb_testCloud"
#from transfer.datasets.BigEarthNet.bigearthnet_dataset_seco import Bigearthnet
#from transfer.datasets.BigEarthNet.bigearthnet_dataset_seco import BigearthnetClouds
#mode=['s1','s2a']
#train_dataset = Bigearthnet(root=datapath, bands=mode, split='train')
#train_dataset = BigearthnetClouds(root=datapath, bands=mode, split='train')
#val_dataset = Bigearthnet(root=datapath, bands=mode, split='validation')
#val_dataset = BigearthnetClouds(root=datapath, bands=mode, split='validation')
#test_dataset = Bigearthnet(root=datapath, bands=mode, split='test')
#test_dataset = BigearthnetClouds(root=datapath, bands=mode, split='test')
#make_lmdb(train_dataset,lmdbpathtrain,num_workers=32)
#make_lmdb(val_dataset,lmdbpathval,num_workers=32)
#make_lmdb(test_dataset,lmdbpathtest,num_workers=4)
def make_lmdb(dataset, lmdb_file, num_workers=6):
    ...
    for index, (s1, s2a, target) in tqdm(enumerate(loader), total=len(dataset), desc='Creating LMDB'):
        sample_s1 = np.array(s1)
        sample_s2a = np.array(s2a)
        obj = (sample_s1.tobytes(), sample_s1.shape, sample_s2a.tobytes(), sample_s2a.shape, target.tobytes())
...
class LMDBDataset(Dataset):
    ...
    def __getitem__(self, index):
        ...
        s1_bytes, s1_shape, s2a_bytes, s2a_shape, target_bytes = pickle.loads(data)
        ...
        sample_s2 = np.frombuffer(s2a_bytes, dtype=np.int16).reshape(s2a_shape)
        sample_s2 = (sample_s2 / 10000.0).astype(np.float32)
        sample_s1 = torch.from_numpy(sample_s1)
        sample_s2 = torch.from_numpy(sample_s2)
        ...
        sample = torch.vstack((sample_s1, sample_s2))
