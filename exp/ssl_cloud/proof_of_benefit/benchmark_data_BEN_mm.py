...
ALL_BANDS = ['B01', 'B02', 'B03', 'B04', 'B05', 'B06', 'B07', 'B08', 'B8A', 'B09', 'B11', 'B12', 'VV', 'VH']
...
class Bigearthnet(Dataset):
    ...
    def __init__(self, root, split, bands=None, transform=None, target_transform=None, download=False, use_new_labels=True, normalize=False):
            ...
            self.samples = []
            df = pd.read_parquet(self.root / "metadata.parquet")
            for index, row in df.iterrows():
                if row["split"] == split:
                    tile_dir = "_".join(row["patch_id"].split("_")[:-2])
                    s1_tile_dir = "_".join(row["s1_name"].split("_")[:-3])
                    optical_path = self.root / "BigEarthNet-S2" / tile_dir / row["patch_id"]
                    radar_path = self.root / "BigEarthNet-S1" / s1_tile_dir / row["s1_name"]
                    self.samples.append(optical_path, radar_path)

    def __getitem__(self, index):
            paths = self.samples[index]
            path_optical = paths[0]
            path_radar = paths[1]
            patch_id_optical = path_optical.name
            patch_id_radar = path_radar.name

            channels = []
            for b in self.bands:
                if "B" in b:
                    ch = rasterio.open(path_optical / f'{patch_id_optical}_{b}.tif').read(1)
                    if self.normalize:
                        ch = normalize(ch, mean=BAND_STATS['mean'][b], std=BAND_STATS['std'][b])
                    ch = cv2.resize(ch, dsize=(128, 128), interpolation=cv2.INTER_CUBIC)
                    channels.append(ch)
                else:
                    ch = rasterio.open(path_radar / f'{patch_id_radar}_{b}.tif').read(1)
                    if self.normalize:
                        ch = normalize(ch, mean=BAND_STATS['mean'][b], std=BAND_STATS['std'][b])
                    ch = cv2.resize(ch, dsize=(128, 128), interpolation=cv2.INTER_CUBIC)
                    channels.append(ch)
            ...
            df = pd.read_parquet(self.root / "metadata.parquet")
            df.loc[df["patch_id"] == patch_id_optical]
            first_row = df.iloc[0]
            target = self.get_multihot_new(first_row["labels"])
