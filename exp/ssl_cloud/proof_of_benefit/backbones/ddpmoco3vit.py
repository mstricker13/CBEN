...
parser.add_argument('--data', metavar='DIR',
                    help='path to dataset', default='/data/Phd_data/Projects/SSL/data/ssl4eo/lmdb_mm/')
parser.add_argument('-a', '--arch', metavar='ARCH', default='vit_small',
                    choices=model_names,
                    help='model architecture: ' +
                        ' | '.join(model_names) +
                        ' (default: resnet50)')  #TODO
...
parser.add_argument('-b', '--batch-size', default=256, type=int,
                    ...
                    help='mini-batch size (default: 4096), this is the total '
                         ...
                         'using Data Parallel or Distributed Data Parallel') #TODO 4096
parser.add_argument('--lr', '--learning-rate', default=1.5e-4, type=float,
                    metavar='LR', help='initial (base) learning rate', dest='lr') #0.6
parser.add_argument('--momentum', default=0.9, type=float, metavar='M',
                    help='momentum')
parser.add_argument('--wd', '--weight-decay', default=.1, type=float,
                    metavar='W', help='weight decay (default: 1e-6)', #1e-6
                    dest='weight_decay')
parser.add_argument('-p', '--print-freq', default=10, type=int,
                    metavar='N', help='print frequency (default: 10)')
parser.add_argument('--resume', default='/data/Phd_data/Projects/SSL/output/ssl4eo/ddpmoco3vit/checkpoint_0065.pth.tar', type=str, metavar='PATH',
                    help='path to latest checkpoint (default: none)')
parser.add_argument('--world-size', default=-1, type=int,
                    help='number of nodes for distributed training')
parser.add_argument('--rank', default=-1, type=int,
                    help='node rank for distributed training')
parser.add_argument('--dist-url', default="file:///code/ddd/ddpmoco3vit", type=str,
                    help='url used to set up distributed training') #TODO 
...
parser.add_argument('--moco-m-cos', action='store_true', default=True,
                    help='gradually increase moco momentum to 1 with a '
                         'half-cycle cosine schedule')
parser.add_argument('--moco-t', default=0.2, type=float,
                    help='softmax temperature (default: 1.0)') #1.0

# vit specific configs:
parser.add_argument('--stop-grad-conv1', action='store_true', default=True,
...
parser.add_argument('--bands', type=str, default='MM', help="input bands")  #TODO
...
parser.add_argument('--checkpoints', metavar='DIR',
                    ...
                    default='/data/Phd_data/Projects/SSL/output/ssl4eo/ddpmoco3vit')



parser.add_argument('--normalize', action='store_true', default=False)
parser.add_argument('--mode', nargs='*', default=['s1','s2a']) #TODO
...
def init_dist(args):
    ...
    if args.dist_url == "env://":
        args.dist_url = "file:///work/ddd/sharedfileMoco3vit"
    shared_file = Path("/work/ddd/sharedfileMoco3vit")
...
def main_worker(gpu, ngpus_per_node, args):
    ...
    if args.arch.startswith('vit'):
        model = moco_builder.MoCo_ViT(
            partial(vits.__dict__[args.arch], stop_grad_conv1=args.stop_grad_conv1, in_chans=n_channels),
    ...
    augmentation1 = [
        transforms.RandomResizedCrop(args.in_size, scale=(args.crop_min, 1.)),
    ...
    train_dataset = LMDBDataset_moco_res50(
        lmdb_file=args.data,
        s12a_transform=TwoCropsTransform(base_transform1=transforms.Compose(augmentation1), base_transform2 = transforms.Compose(augmentation2),season=args.season),
