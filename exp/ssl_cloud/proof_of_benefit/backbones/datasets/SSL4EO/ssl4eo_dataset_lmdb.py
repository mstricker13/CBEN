...
#python
#from backbones.datasets.SSL4EO.ssl4eo_dataset_lmdb import make_lmdb
#datapath = "C:/Users/m_str/Documents/Phd_data/Projects/SSL/data/ssl4eo/ssl4eo-s12_100patches/"
#datapath = "/data/Phd_data/Projects/SSL/data/ssl4eo/ssl4eo-s12/"
#lmdbpath = "C:/Users/m_str/Documents/Phd_data/Projects/SSL/data/ssl4eo/lmdb_mm"
#lmdbpath = "/data/Phd_data/Projects/SSL/data/ssl4eo/lmdb_mm"
#mode=['s1','s2a']
#from backbones.datasets.SSL4EO.ssl4eo_dataset import SSL4EO
#train_dataset = SSL4EO(root=datapath, normalize=False, mode=mode, dtype='uint8')
#make_lmdb(train_dataset,lmdbpath,num_workers=6,mode=mode)
def make_lmdb(dataset, lmdb_file, num_workers=6,mode=['s1','s2a','s2c']):
    ...
    print("RUNNING")
    #env = lmdb.open(lmdb_file, map_size=int(1e9))
    ...
    for index, (s1, s2a, s2c) in tqdm(enumerate(loader), total=len(dataset), desc='Creating LMDB'):
        ...
        if mode==['s1','s2a','s2c']:
            ...
            obj = (sample_s1.tobytes(), sample_s1.shape, sample_s2a.tobytes(), sample_s2a.shape)
...
class LMDBDataset(Dataset):
    ...
    def __getitem__(self, index):
        ...
        if self.mode==['s1','s2a']:
            s1_bytes, s1_shape, s2a_bytes, s2a_shape = pickle.loads(data)
            if self.dtype=='uint8':
                sample_s1 = np.frombuffer(s1_bytes, dtype=np.uint8).reshape(s1_shape)
                sample_s2 = np.frombuffer(s2a_bytes, dtype=np.uint8).reshape(s2a_shape)
            else:
                sample_s1 = np.frombuffer(s1_bytes, dtype=np.float32).reshape(s1_shape)
            ...
            if self.s1_transform is not None:
                sample_s1 = self.s1_transform(sample_s1)
            if self.s2a_transform is not None:
                sample_s2 = self.s2a_transform(sample_s2)
            return torch.vstack((sample_s1, sample_s2))
...
class LMDBDataset_moco_res50(Dataset):

    def __init__(self, lmdb_file, is_slurm_job=False, s1_transform=None, s2a_transform=None, s2c_transform=None, s12a_transform=None, subset=None, normalize=False, mode=['s1','s2a','s2c'], dtype='raw'):
        self.lmdb_file = lmdb_file
        self.s1_transform = s1_transform
        self.s2a_transform = s2a_transform
        self.s2c_transform = s2c_transform
        self.s12a_transform = s12a_transform
        self.is_slurm_job = True
        self.subset = subset
        self.normalize = normalize
        self.mode = mode
    ...
    def __getitem__(self, index):
        ...
        if self.mode==['s1','s2a']:
            ...
            if self.dtype=='uint8':
                ...
                sample_s2 = np.frombuffer(s2a_bytes, dtype=np.int16).reshape(s2a_shape)
                sample_s2 = (sample_s2 / 10000.0).astype(np.float32)
            if self.s12a_transform is not None:
                sample_stacked = np.hstack((sample_s1, sample_s2))
                sample_stacked = self.s12a_transform(sample_stacked)
            return sample_stacked
